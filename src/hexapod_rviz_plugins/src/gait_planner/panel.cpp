...
#include <cstddef>
#include <functional>
#include <hexapod_msgs/msg/hexapod_gait.hpp>
#include <memory>
...
void HexapodControlRvizPanel::setupROS() {
  ...
  pose_sub_ = node_->create_subscription<hexapod_msgs::msg::LegPoses>(
  ...
  hexapod_gait_sub_ =
      node_->create_subscription<hexapod_msgs::msg::HexapodGait>(
          "/hexapod/gait", 10,
          std::bind(&HexapodControlRvizPanel::hexapodGaitUpdateCallback, this,
                    std::placeholders::_1));
}

void HexapodControlRvizPanel::hexapodGaitUpdateCallback(
    const hexapod_msgs::msg::HexapodGait msg) {
  RCLCPP_INFO(node_->get_logger(), "Got Gait Message %s", msg.name.c_str());
  gait_ = msg;
}
